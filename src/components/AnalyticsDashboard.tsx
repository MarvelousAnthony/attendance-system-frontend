  ...
  const fetchStudents = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error("Failed to retrieve student records from server");
      }
    } catch (err: any) {
      console.error(err);
    }
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-850 rounded-xl pl-4 pr-10 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500 transition-all font-semibold"
                  required
                />
                <button
                  type="button"
